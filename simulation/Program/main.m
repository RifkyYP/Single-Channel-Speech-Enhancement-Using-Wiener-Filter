...
dB = ["0", "5", "10", "15"];
method = ["wiener", "SS"];
[clean_speech, fs]= audioread('..\male\sp24.wav');
aInfo = audioinfo('..\male\sp24.wav');
nbits = aInfo.BitsPerSample; % 16 bits resolution
for i = 1:length(environment)
    for j = 1:length(dB)
        infile = strcat('..\male\', environment(i), '\in\sp24_', ... 
            environment(i), '_sn', dB(j), '.wav');
        [noisy_speech{i,j}, fs]= audioread(infile);
        for k = 1:length(method)
            outfile = strcat('..\male\', environment(i), '\out\', method(k), ...
                '_sp24_', environment(i), '_sn', dB(j), '.wav');
            if k == 1
                [enhanced_wiener{i,j}, fs, nbits] = wiener_as(infile, outfile);
            elseif k == 2
                [enhanced_ss{i,j}, Srate] = specsub(infile, outfile);
            end
            fwSNRseg((i-1)*length(dB)+j,k) = comp_fwseg('..\male\sp24.wav', outfile);
            LLR((i-1)*length(dB)+j,k) = comp_llr('..\male\sp24.wav', outfile);
            PESQ((i-1)*length(dB)+j,k) = pesq('..\male\sp24.wav', outfile);
        end
    end
end
